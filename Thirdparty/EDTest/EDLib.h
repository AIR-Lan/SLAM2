...
/// Note: smoothingSigma must be >= 1.0
    EdgeMap *DetectEdgesByED(unsigned char *srcImg, int width, int height, GradientOperator op, int GRADIENT_THRESH,
...
/// Note: smoothingSigma must be >= 1.0
    EdgeMap *DetectEdgesByCannySR(unsigned char *srcImg, int width, int height, int cannyLowThresh, int cannyHighThresh,
                                  int sobelKernelApertureSize = 3, double smoothingSigma = 1.0);
...
/// (2) Validate the edge segments using the Helmholtz principle, returning only the validated edge segments
/// Note: smoothingSigma must be >= 1.0
    EdgeMap *DetectEdgesByCannySRPF(unsigned char *srcImg, int width, int height, int sobelKernelApertureSize = 3,
