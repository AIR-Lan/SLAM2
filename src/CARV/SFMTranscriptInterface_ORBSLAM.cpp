...
//段代码是一个SFM（Structure-from-Motion）转录接口的实现，特定于ORBSLAM算法。
// SFMTranscriptInterface_ORBSLAM类用于记录ORBSLAM的执行过程，包括关键帧、地图点和线段的插入、删除、移动等操作，以及进行bundle adjustment（束调整）的记录。
...
void SFMTranscriptInterface_ORBSLAM::addPointDeletionEntry(ORB_SLAM3::MapPoint *p){
    try{
        std::stringstream ssTmp;

        // Set nPointIndex based on argument.
        std::map<ORB_SLAM3::MapPoint *, int>::iterator itMapPoint = m_mMapPoint_Index.find(p);
...
void SFMTranscriptInterface_ORBSLAM::addVisibilityRayInsertionEntry(ORB_SLAM3::KeyFrame *k, ORB_SLAM3::MapPoint *p){
    try{
        if(! m_bSuppressRefindLogging){
            std::stringstream ssTmp;

            // Set nCamIndex and nPointIndex based on arguments.
            std::map<ORB_SLAM3::KeyFrame *, int>::iterator itKeyFrame = m_mKeyFrame_Index.find(k);
...
void SFMTranscriptInterface_ORBSLAM::addVisibilityRayDeletionEntry(ORB_SLAM3::KeyFrame *k, ORB_SLAM3::MapPoint *p){
    try{
        std::stringstream ssTmp;
...
void SFMTranscriptInterface_ORBSLAM::addLineSegmentInsertionEntry(ORB_SLAM3::KeyFrame *kf, Cluster *pCluster){
    try{
        std::stringstream ssTmp;
...
void SFMTranscriptInterface_ORBSLAM::addLineSegmentKeyFrameInsertionEntry(ORB_SLAM3::KeyFrame *kf){
    try{
        std::stringstream ssTmp;
...
void SFMTranscriptInterface_ORBSLAM::addSemiDenseKeyFrameInsertionEntry(ORB_SLAM3::KeyFrame *kf){
    try{
        std::stringstream ssTmp;
        dlovi::Matrix matNewCam(3, 1);
...
void SFMTranscriptInterface_ORBSLAM::addFirstKeyFrameInsertionEntry(ORB_SLAM3::KeyFrame *k){
    try{
        std::stringstream ssTmp;
        dlovi::Matrix matNewCam(3, 1);
        dlovi::Matrix matNewPoint(3, 1);
        ...
        vector<ORB_SLAM3::MapPoint *> vMP;
        m_mKeyFrame_MapPoint[k] = vMP;

        std::set<ORB_SLAM3::MapPoint*> mvpMapPoints = k->GetMapPoints();
        // Process new points and visibility information in this KF
        for(std::set<ORB_SLAM3::MapPoint*>::iterator it = mvpMapPoints.begin(); it != mvpMapPoints.end(); it++){
            ORB_SLAM3::MapPoint * point = *it;
            if(point->isBad())
                continue;
...
void SFMTranscriptInterface_ORBSLAM::addKeyFrameInsertionEntry(ORB_SLAM3::KeyFrame *k){
    try{
        std::stringstream ssTmp;
        dlovi::Matrix matNewCam(3, 1);
        dlovi::Matrix matNewPoint(3, 1);
...
//                std::map<ORB_SLAM3::KeyFrame*, size_t> mObservations = point->GetObservations();
                std::map<ORB_SLAM3::KeyFrame*, std::tuple<int,int>>  mObservations = point->GetObservations();

                bool hasObservation = false;
                for(std::map<ORB_SLAM3::KeyFrame*, std::tuple<int,int>>::iterator it2 = mObservations.begin(); it2 != mObservations.end(); it2++){
                ...
                if (hasObservation) {
                    ...
                    for (std::map<ORB_SLAM3::KeyFrame*, std::tuple<int,int>>::iterator it2 = mObservations.begin();
...
void SFMTranscriptInterface_ORBSLAM::addBundleAdjustmentEntry(set<ORB_SLAM3::KeyFrame *> & sAdjustSet, set<ORB_SLAM3::MapPoint *> & sMapPoints){
    try{
        if(! m_bSuppressBundleAdjustmentLogging){
            ...
            for(set<ORB_SLAM3::MapPoint *>::iterator it = sMapPoints.begin(); it != sMapPoints.end(); it++){
            ...
            // Log KF-move entries
            for(set<ORB_SLAM3::KeyFrame *>::iterator it = sAdjustSet.begin(); it != sAdjustSet.end(); it++){
...
std::vector<ORB_SLAM3::MapPoint *> SFMTranscriptInterface_ORBSLAM::GetNewPoints(ORB_SLAM3::KeyFrame *pKF) {
    try{
