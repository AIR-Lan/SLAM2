...
namespace ORB_SLAM3 {
            ...
            for (size_t y = 0; y < (size_t) kf->im_.rows; y++) {
                for (size_t x = 0; x < (size_t) kf->im_.cols; x++) {
                    ...
                    if (kf->depth_map_checked_.at<float>(y, x) > 0.000001) {
                        ...
                        fileOut << "v " + std::to_string(Pw[0]) + " " + std::to_string(Pw[1]) + " " +
                                   std::to_string(Pw[2]) + " "
                                   + std::to_string(vr) + " " << std::to_string(vg) + " " + std::to_string(vb)
                                << std::endl;
                    }
                }
            }
    ...
    ProbabilityMapping::ProbabilityMapping(ORB_SLAM3::Atlas *pAtlas) {
        mpAtlas = pAtlas;
        mbFinishRequested = false; //init
        mbFinished = false;
        mpModeler = new Modeler(mpAtlas);
...
//TODO BUG mpMap>>mpAtlas
        ...
        duration *= 1000;
        std::cout << "semi dense mapping took total: " << duration << "ms  avg:" << duration / vpKFs.size()
                  << "ms  #KF:" << vpKFs.size() << std::endl;
        ...
        duration += (finish.tv_nsec - start.tv_nsec) / 1000000000.0;
        duration *= 1000;
        std::cout << "modeling took total: " << duration << "ms  avg:" << duration / vpKFs.size() << "ms  #KF:"
                  << vpKFs.size() << std::endl;
        ...
        for (size_t i = 0; i < vpKFs.size(); i++) {
            ...
            for (size_t idxCov = 0; idxCov < closestMatchesAll.size(); idxCov++) {
                if (closestMatches.size() >= covisN)
                    break;
                ORB_SLAM3::KeyFrame *kfCM = closestMatchesAll[idxCov];
...
//        IntraKeyFrameDepthGrowing( kf->depth_map_,  kf->depth_sigma_, kf->GradImg);
    ...
    void ProbabilityMapping::ResetIfRequested() {
        unique_lock<mutex> lock(mMutexReset);
        if (mbResetRequested) {
            Modeler *newModeler = new Modeler(mpAtlas);
            mpAtlas->SetModeler(newModeler);
    ...
    void ProbabilityMapping::ApplySigmaThreshold(ORB_SLAM3::KeyFrame *kf) {
...
#pragma omp parallel for schedule(dynamic) collapse(2)
    ...
    void
    ProbabilityMapping::EpipolarSearch(ORB_SLAM3::KeyFrame *kf1, ORB_SLAM3::KeyFrame *kf2, const int x, const int y,
                                       float pixel,
                                       float min_depth, float max_depth, depthHo *dh, cv::Mat F12, float &best_u,
                                       float &best_v, float th_pi, float rot) {
        ...
        if ((a / b) < -4 || a / b > 4)
            return;   // if epipolar direction is approximate to perpendicular, we discard it.  May be product wrong match.
        ...
        for (int uj = (int) std::ceil(umin); uj <= std::floor(umax); uj++) {
            ...
            float err = (photometric_err * photometric_err +
                         (gradient_modulo_err * gradient_modulo_err) / (float) THETA);
            if (err < old_err) {
                best_pixel = uj;
                old_err = err;
        ...
        if (old_err < 100000.0) {
            ...
            ustar = best_pixel +
                    (g * best_photometric_err + (1 / (float) THETA) * q * best_gradient_modulo_err) / denomiator;
    ...
    void
    ProbabilityMapping::IntraKeyFrameDepthChecking(cv::Mat &depth_map, cv::Mat &depth_sigma, const cv::Mat gradimg) {
...
#pragma omp parallel for schedule(dynamic) collapse(2)
                    ...
                    for (int y = py - 1; y <= py + 1; y++) {
                        for (int x = px - 1; x <= px + 1; x++) {
                            ...
                            if (depth_map.at<float>(y, x) > 0.000001) {
                                if (ChiTest(depth_map.at<float>(y, x), depth_map.at<float>(py, px),
                                            depth_sigma.at<float>(y, x), depth_sigma.at<float>(py, px))) {
    ...
    void
    ProbabilityMapping::IntraKeyFrameDepthGrowing(cv::Mat &depth_map, cv::Mat &depth_sigma, const cv::Mat gradimg) {
        cv::Mat depth_map_new = depth_map.clone();
        cv::Mat depth_sigma_new = depth_sigma.clone();
...
#pragma omp parallel for schedule(dynamic) collapse(2)
        for (int py = 2; py < (depth_map.rows - 2); py++) {
            for (int px = 2; px < (depth_map.cols - 2); px++) {
                ...
                if (depth_map.at<float>(py, px) <
                    0.000001)  // if  d ==0.0 : grow the reconstruction getting more density
                {
    ...
    void ProbabilityMapping::InterKeyFrameDepthChecking(const cv::Mat &im, ORB_SLAM3::KeyFrame *currentKf,
    ...
    void ProbabilityMapping::InterKeyFrameDepthChecking(ORB_SLAM3::KeyFrame *currentKf,
                                                        std::vector<ORB_SLAM3::KeyFrame *> neighbors) {
...
#pragma omp parallel for schedule(dynamic) collapse(2)
        for (int py = 2; py < rows - 2; py++) {
            for (int px = 2; px < cols - 2; px++) {
                ...
                for (size_t j = 0; j < neighbors.size(); j++) {
                    ...
                    cv::Mat xp = (cv::Mat_<float>(3, 1) << (px - cx) / fx, (py - cy) /
                                                                           fy, 1.0);// inverse project.    if has distortion, this code shoud fix
                ...
                if (compatible_neighbor_keyframes_count < lambdaN) {
                    ...
                    for (size_t j = 0; j < compatible_pixels_by_frame.size(); j++) {
                        ...
                        for (size_t n = 0; n < compatibleJ.size(); n++) {
                            ...
                            r0.at<float>(idxJN, 0) =
                                    (djn - dp * rzxp.at<float>(0, 0) - tji[j].at<float>(2, 0)) / d2sigma;
    ...
    void
    ProbabilityMapping::Equation14(depthHo &dHjn, float &depthp, cv::Mat &xp, cv::Mat &rji, cv::Mat &tji, float *res) {
        cv::Mat tempm = rji.row(2) * xp;
        float tempf = tempm.at<float>(0, 0);
...
////////////////////////
    ...
    void ProbabilityMapping::ComputeInvDepthHypothesis(ORB_SLAM3::KeyFrame *kf, ORB_SLAM3::KeyFrame *kf2, float ustar,
                                                       float ustar_var,
                                                       float a, float b, float c, ProbabilityMapping::depthHo *dh,
                                                       int x, int y) {
    ...
    void ProbabilityMapping::GetGradientMagAndOri(const cv::Mat &image, cv::Mat *gradx, cv::Mat *grady, cv::Mat *mag,
                                                  cv::Mat *ori) {
...
//might be a good idea to store these when they get calculated during ORB-SLAM.
    void ProbabilityMapping::GetInPlaneRotation(ORB_SLAM3::KeyFrame *k1, ORB_SLAM3::KeyFrame *k2, float *th) {
        std::vector<cv::KeyPoint> vKPU1 = k1->GetKeyPointsUn();
        DBoW2::FeatureVector vFeatVec1 = k1->GetFeatureVector();
        std::vector<ORB_SLAM3::MapPoint *> vMapPoints1 = k1->GetMapPointMatches();
        ...
        std::vector<ORB_SLAM3::MapPoint *> vMapPoints2 = k2->GetMapPointMatches();
        ...
        while (f1it != f1end && f2it != f2end) {
            if (f1it->first == f2it->first) {
                for (size_t i1 = 0, iend1 = f1it->second.size(); i1 < iend1; i1++) {
                    ...
                    ORB_SLAM3::MapPoint *pMP1 = vMapPoints1[index1];
                    if (!pMP1)
                        continue;
                    if (pMP1->isBad())
                        continue;
                    ...
                    for (size_t i2 = 0, iend2 = f2it->second.size(); i2 < iend2; i2++) {
                        ...
                        ORB_SLAM3::MapPoint *pMP2 = vMapPoints2[index2];
                        if (!pMP2)
                            continue;
                        if (pMP2->isBad())
                            continue;
                        ...
                        int dist = ORB_SLAM3::ORBmatcher::DescriptorDistance(d1, d2);
    ...
    void ProbabilityMapping::PixelNeighborSupport(std::vector<std::vector<depthHo> > H, int px, int py,
                                                  std::vector<depthHo> &support) {
        support.clear();
        float chi = 0;
        for (int y = py - 1; y <= py + 1; y++) {
            for (int x = px - 1; x <= px + 1; x++) {
    ...
    void ProbabilityMapping::PixelNeighborNeighborSupport(std::vector<std::vector<depthHo> > H, int px, int py,
    ...
    void
    ProbabilityMapping::GetIntensityGradient_D(const cv::Mat &ImGrad, float a, float b, float c, int px, float *q) {
        int uplusone = px + 1;
    ...
    void ProbabilityMapping::GetTR(ORB_SLAM3::KeyFrame *kf, cv::Mat *t, cv::Mat *r) {
    ...
    void ProbabilityMapping::GetParameterization(const cv::Mat &F12, const int x, const int y, float &a, float &b,
                                                 float &c) {
...
// Linear Triangulation Method
    void ProbabilityMapping::GetPixelDepth(float uj, float vj, int px, int py, ORB_SLAM3::KeyFrame *kf,
                                           ORB_SLAM3::KeyFrame *kf2, float &p, ProbabilityMapping::depthHo *dh) {
...
// Equation (8)
    void ProbabilityMapping::GetPixelDepth(float uj, int px, int py, ORB_SLAM3::KeyFrame *kf, ORB_SLAM3::KeyFrame *kf2,
                                           float &p) {
    ...
    void ProbabilityMapping::GetSearchRange(float &umin, float &umax, int px, int py, float mind, float maxd,
        ...
        cv::Mat xp1 = (cv::Mat_<float>(3, 1) << (px - cx) / fx, (py - cy) /
                                                                fy, 1.0);  // inverse project.    if has distortion, this code shoud fix
    ...
    void
    ProbabilityMapping::GetFusion(const std::vector<std::pair<float, float> > supported, float &depth, float &sigma) {
...
//    sigma = sqrt(1 / rsj);
    ...
    void
    ProbabilityMapping::GetFusion(const std::vector<depthHo> &compatible_ho, depthHo &hypothesis, float *min_sigma) {
        hypothesis.depth = 0;
        hypothesis.sigma = 0;
