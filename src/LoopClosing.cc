...
void LoopClosing::CorrectLoopWithLines()
{
    ...
    // 对地图点操作
    {
        ...
        for(vector<KeyFrame*>::iterator vit=mvpCurrentConnectedKFs.begin(), vend=mvpCurrentConnectedKFs.end(); vit!=vend; vit++)
        {
            ...
            if(pKFi!=mpCurrentKF)// 跳过当前关键帧，因为当前关键帧的位姿已经在前面优化过了
            {
                cv::Mat Tic = Tiw*Twc;
...
void LoopClosing::MergeLocalWithLines()
{
    ...
    for(KeyFrame* pKFi : spLocalWindowKFs)
    {
        ...
        if(pKFi!=mpCurrentKF)
        {
            ...
            // 根据链式法则,利用当前关键帧和第i个共视关键帧的位姿关系,以及当前关键帧矫正后的初始位姿,
            cv::Mat Tic = Tiw*Twc;
            cv::Mat Ric = Tic.rowRange(0,3).colRange(0,3);
            cv::Mat tic = Tic.rowRange(0,3).col(3);
            g2o::Sim3 g2oSic(Converter::toMatrix3d(Ric),Converter::toVector3d(tic),1.0);
        ...
        Eigen::Matrix3d eigR = g2oCorrectedSiw.rotation().toRotationMatrix();
        Eigen::Vector3d eigt = g2oCorrectedSiw.translation();
        // 根据上面计算得到的融合矫正后的初始位姿(Sim3),更新窗口内每个关键帧的mTcwMerge(矫正后的初始
        double s = g2oCorrectedSiw.scale();
...
#endif
    // Step 3 两个地图以新（当前帧所在地图）换旧（融合帧所在地图），包括关键帧及地图点关联地图的以新换旧
    {
        // 锁住当前地图
    ...
    {
        // 运行的 welding BA , 优化所有的当前关键帧共视窗口里的关键帧和地图点,
    ...
    else
    {
        ...
        {    // 锁住当前地图
            ...
            {
                unique_lock<mutex> currentLock(pCurrentMap->mMutexMapUpdate); // We update the current map with the Merge information
                // Step 8.1 拿到当前帧所在地图里的所有关键帧, 重复之前的过程, 不过这里是对于地图里剩余的所
