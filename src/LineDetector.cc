...
int line3D_covisN = 10;
    ...
    //TODO
/// Function prototype for exported by EDLinesLib.a
    ...
    //myplan 用于在离线模式下进行线段拟合和重建的函数，
//通过遍历关键帧向量，对每个关键帧的线段进行拟合和重建，
// 将重建后的线段的起始点和结束点的世界坐标写入输出文件。代码还包括了计时和输出运行时间
void LineDetector::LineFittingEDLinesOffline(std::vector<ORB_SLAM3::KeyFrame *> vpKFs) {
    ...
    //myplan
//通过遍历关键帧向量，将关键帧的线段信息和其他相关信息传递给 Line3D++ 对象，
// 然后运行图像匹配和线段重建算法，并将结果保存到文件中。代码还包括了计时和输出运行时间的功能。
    void LineDetector::RunLine3Dpp(std::vector<ORB_SLAM3::KeyFrame *> vpKFs) {
        struct timespec start, finish;
        double duration;
...
//通过遍历关键帧向量，对每个关键帧的线段进行拟合和重建，
// 将重建后的线段的起始点和结束点的世界坐标写入输出文件。代码还包括了计时和输出运行时间
    void LineDetector::LineFit(Pixel *pixelChain, int noPixels, ORB_SLAM3::KeyFrame* kf)
    {
...
//myplan 离线模式下进行线段拟合和合并的函数
//遍历关键帧向量，对每个关键帧的线段进行拟合和合并
    void LineDetector::LineFittingOffline(std::vector<ORB_SLAM3::KeyFrame *> vpKFs, ORB_SLAM3::Modeler* pModeler) {
...
//算给定像素链中具有有效深度的像素数量的函数
    int LineDetector::CountDepth(Pixel *pixelChain, int length, ORB_SLAM3::KeyFrame* kf)
    {
        int count = 0;
...
//myplan
//执行最小二乘拟合，根据像素链和深度信息，拟合出直线的深度与像素距离之间的关系，并计算拟合误差
    void LineDetector::LeastSquaresDepthFit(Pixel* pixelChain, int length, float la, float lb, float lc, float& u1, float& u2, float& depthFitError, ORB_SLAM3::KeyFrame* kf)
    {
    ...
    float LineDetector::ComputePointDepth2Line(float a, float b, float c, float alpha, float beta, Pixel *pixelChain, Pixel pixel, ORB_SLAM3::KeyFrame* kf)
    {
        int x = pixel.c;
        int y = pixel.r;
...
//myplan 半稠密建图用
    void LineDetector::DetectEdgeMap(ORB_SLAM3::KeyFrame *kf) {
    ...
    void LineDetector::LineFitting(ORB_SLAM3::KeyFrame* kf)
    {
        struct timespec start, finish;
        double duration;
...
//myplan
    void LineDetector::MergeLines(ORB_SLAM3::KeyFrame* kf, ORB_SLAM3::Modeler* pModeler)
    {
...
//检测图像中的直线
    void LineDetector::DetectLineSegments(ORB_SLAM3::KeyFrame* kf) {
