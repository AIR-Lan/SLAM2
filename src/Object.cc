...
// BRIEF 2d objects (in the frame) associate with 3d objects (in the map).
    void Object_2D::ObjectDataAssociation(Atlas *mpAtlas, Frame &mCurrentFrame, cv::Mat &image, string &flag)
    {
        if(flag == "None")
            biForest = false;
        ...
        if((flag != "NA") && (flag != "NP"))
        {
            ...
            {
                if (_class_id != mpAtlas->mvObjectMap[i]->mnClass)
                    continue;

                if (mpAtlas->mvObjectMap[i]->bBadErase)
                    continue;

                if (mpAtlas->mvObjectMap[i]->mnLastAddID == mCurrentFrame.mnId - 1)
                {
                    // step 1.1 predict object bounding box according to last frame and next to last frame.
                    if (mpAtlas->mvObjectMap[i]->mnLastLastAddID == mCurrentFrame.mnId - 2)
                    {
                        // left-top.
                        float left_top_x = mpAtlas->mvObjectMap[i]->mLastRect.x * 2 - mpAtlas->mvObjectMap[i]->mLastLastRect.x;
                        if (left_top_x < 0)
                            left_top_x = 0;
                        float left_top_y = mpAtlas->mvObjectMap[i]->mLastRect.y * 2 - mpAtlas->mvObjectMap[i]->mLastLastRect.y;
                        if (left_top_y < 0)
                            left_top_y = 0;

                        // right-bottom.
                        float right_down_x = (mpAtlas->mvObjectMap[i]->mLastRect.x + mpAtlas->mvObjectMap[i]->mLastRect.width) * 2 - (mpAtlas->mvObjectMap[i]->mLastLastRect.x + mpAtlas->mvObjectMap[i]->mLastLastRect.width);
                        if (left_top_x > image.cols)
                            right_down_x = image.cols;
                        float right_down_y = (mpAtlas->mvObjectMap[i]->mLastRect.y + mpAtlas->mvObjectMap[i]->mLastRect.height) * 2 - (mpAtlas->mvObjectMap[i]->mLastLastRect.y + mpAtlas->mvObjectMap[i]->mLastLastRect.height);
                        ...
                        RectPredict = mpAtlas->mvObjectMap[i]->mLastRect;
            ...
            {
                // update.
                bool bFlag = mpAtlas->mvObjectMap[IouMaxObjID]->DataAssociateUpdate(this, mCurrentFrame, image, 1);
        ...
        if((flag != "NA") && (flag != "IoU"))
        {
            for (int i = (int)mpAtlas->mvObjectMap.size() - 1; (i >= 0) && (old == false); i--)
            {
                if (_class_id != mpAtlas->mvObjectMap[i]->mnClass)
                    continue;
                ...
                int nFlag = this->NoParaDataAssociation(mpAtlas->mvObjectMap[i], mCurrentFrame, image);
            ...
            if (vObjByNPId.size() >= 1)
            {
                ...
                if (bAssoByIou)
                {
                    for (int i = 0; i < vObjByNPId.size(); i++)
                    {
                        ...
                        sit = mpAtlas->mvObjectMap[nAssoByIouId]->mReObj.find(mpAtlas->mvObjectMap[vObjByNPId[i]]->mnId);
                        if (sit != mpAtlas->mvObjectMap[nAssoByIouId]->mReObj.end())
                        {
                            int sit_sec = sit->second;
                            mpAtlas->mvObjectMap[nAssoByIouId]->mReObj.erase(mpAtlas->mvObjectMap[vObjByNPId[i]]->mnId);
                            mpAtlas->mvObjectMap[nAssoByIouId]->mReObj.insert(make_pair(mpAtlas->mvObjectMap[vObjByNPId[i]]->mnId, sit_sec + 1));
                        }
                        else
                            mpAtlas->mvObjectMap[nAssoByIouId]->mReObj.insert(make_pair(mpAtlas->mvObjectMap[vObjByNPId[i]]->mnId, 1));
                    }
                }
                ...
                else
                {
                    ...
                    {
                        // update.
                        bool bFlag = mpAtlas->mvObjectMap[vObjByNPId[i]]->DataAssociateUpdate(this, mCurrentFrame, image, 2); // 2: NP.
                        ...
                        if (bFlag)
                        {
                            ...
                            if (vObjByNPId.size() > i + 1)
                            {
                                for (int j = i + 1; j < vObjByNPId.size(); j++)
                                {
                                    // Record potential association objects and potential association times.
                                    map<int, int>::iterator sit;
                                    sit = mpAtlas->mvObjectMap[vObjByNPId[i]]->mReObj.find(mpAtlas->mvObjectMap[vObjByNPId[j]]->mnId);
                                    if (sit != mpAtlas->mvObjectMap[vObjByNPId[i]]->mReObj.end())
                                    {
                                        int sit_sec = sit->second;
                                        mpAtlas->mvObjectMap[vObjByNPId[i]]->mReObj.erase(mpAtlas->mvObjectMap[vObjByNPId[j]]->mnId);
                                        mpAtlas->mvObjectMap[vObjByNPId[i]]->mReObj.insert(make_pair(mpAtlas->mvObjectMap[vObjByNPId[j]]->mnId, sit_sec + 1));
                                    }
                                    else
                                        mpAtlas->mvObjectMap[vObjByNPId[i]]->mReObj.insert(make_pair(mpAtlas->mvObjectMap[vObjByNPId[j]]->mnId, 1));
                                }
                                break;
                            }
                        }
        ...
        if (old == false)
        {
            ...
            if((flag != "NA") && (flag != "IoU") && (flag != "NP"))
            {
                ...
                for (int i = (int)mpAtlas->mvObjectMap.size() - 1; i >= 0; i--)
                {
                    ...
                    float fIou = Converter::bboxOverlapratio(RectCurrent, mpAtlas->mvObjectMap[i]->mRectProject);
                    float fIou2 = Converter::bboxOverlapratio(mRectFeaturePoints, mpAtlas->mvObjectMap[i]->mRectProject);
                    fIou = max(fIou, fIou2);
                ...
                if (fIouMax >= 0.25)
                {
                    ...
                    if (bAssoByIou || bAssoByNp)
                    {
                        for (int j = vObjByProIouId.size() - 1; j >= 0; j--)
                        {
                            ...
                            map<int, int>::iterator sit;
                            sit = mpAtlas->mvObjectMap[ReId]->mReObj.find(mpAtlas->mvObjectMap[vObjByProIouId[j]]->mnId);
                            if (sit != mpAtlas->mvObjectMap[ReId]->mReObj.end())
                            {
                                int sit_sec = sit->second;
                                mpAtlas->mvObjectMap[ReId]->mReObj.erase(mpAtlas->mvObjectMap[vObjByProIouId[j]]->mnId);
                                mpAtlas->mvObjectMap[ReId]->mReObj.insert(make_pair(mpAtlas->mvObjectMap[vObjByProIouId[j]]->mnId, sit_sec + 1));
                            }
                            else
                                mpAtlas->mvObjectMap[ReId]->mReObj.insert(make_pair(mpAtlas->mvObjectMap[vObjByProIouId[j]]->mnId, 1));
                        }
                    }
                    else
                    {
                        // update.
                        bool bFlag = mpAtlas->mvObjectMap[ProIouMaxObjId]->DataAssociateUpdate(this, mCurrentFrame, image, 4); // 4: project iou.
                        ...
                        for (int j = vObjByProIouId.size() - 1; j >= 0; j--)
                        {
                            ...
                            map<int, int>::iterator sit;
                            sit = mpAtlas->mvObjectMap[ProIouMaxObjId]->mReObj.find(mpAtlas->mvObjectMap[vObjByProIouId[j]]->mnId);
                            if (sit != mpAtlas->mvObjectMap[ProIouMaxObjId]->mReObj.end())
                            {
                                int sit_sec = sit->second;
                                mpAtlas->mvObjectMap[ProIouMaxObjId]->mReObj.erase(mpAtlas->mvObjectMap[vObjByProIouId[j]]->mnId);
                                mpAtlas->mvObjectMap[ProIouMaxObjId]->mReObj.insert(make_pair(mpAtlas->mvObjectMap[vObjByProIouId[j]]->mnId, sit_sec + 1));
                            }
                            else
                                mpAtlas->mvObjectMap[ProIouMaxObjId]->mReObj.insert(make_pair(mpAtlas->mvObjectMap[vObjByProIouId[j]]->mnId, 1));
                        }
                    }
                }
            }
            ...
            if((flag != "NA") && (flag != "IoU") && (flag != "NP"))
            {
                for (int i = (int)mpAtlas->mvObjectMap.size() - 1; i >= 0; i--)
                {
                    ...
                    dis_x = abs(mpAtlas->mvObjectMap[i]->mCenter3D.at<float>(0, 0) - _Pos.at<float>(0, 0));
                    dis_y = abs(mpAtlas->mvObjectMap[i]->mCenter3D.at<float>(1, 0) - _Pos.at<float>(1, 0));
                    dis_z = abs(mpAtlas->mvObjectMap[i]->mCenter3D.at<float>(2, 0) - _Pos.at<float>(2, 0));

                    // t-test.
                    t_test_x = dis_x / (mpAtlas->mvObjectMap[i]->mCenterStandar_x / sqrt(df));
                    t_test_y = dis_y / (mpAtlas->mvObjectMap[i]->mCenterStandar_y / sqrt(df));
                    t_test_z = dis_z / (mpAtlas->mvObjectMap[i]->mCenterStandar_z / sqrt(df));
                    ...
                    else if ((t_test_x + t_test_y + t_test_z) / 3 < 4)
                    {
                        ...
                        float fIou_force = Converter::bboxOverlapratio(RectCurrent, mpAtlas->mvObjectMap[i]->mRectProject);
                        float fIou2_force = Converter::bboxOverlapratio(mRectFeaturePoints, mpAtlas->mvObjectMap[i]->mRectProject);
                ...
                if (bAssoByIou || bAssoByNp || bAssoByProject)
                {
                    ...
                    if (vObjByTId.size() >= 1)
                    {
                        for (int j = 0; j < vObjByTId.size(); j++)
                        {
                            ...
                            map<int, int>::iterator sit;
                            sit = mpAtlas->mvObjectMap[ReId]->mReObj.find(mpAtlas->mvObjectMap[vObjByTId[j]]->mnId);
                            if (sit != mpAtlas->mvObjectMap[ReId]->mReObj.end())
                            {
                                int sit_sec = sit->second;
                                mpAtlas->mvObjectMap[ReId]->mReObj.erase(mpAtlas->mvObjectMap[vObjByTId[j]]->mnId);
                                mpAtlas->mvObjectMap[ReId]->mReObj.insert(make_pair(mpAtlas->mvObjectMap[vObjByTId[j]]->mnId, sit_sec + 1));
                            }
                            else
                                mpAtlas->mvObjectMap[ReId]->mReObj.insert(make_pair(mpAtlas->mvObjectMap[vObjByTId[j]]->mnId, 1));
                        }
                    }
                    ...
                    if (vObjByTIdLower.size() >= 0)
                    {
                        for (int j = 0; j < vObjByTIdLower.size(); j++)
                        {
                            ...
                            map<int, int>::iterator sit;
                            sit = mpAtlas->mvObjectMap[ReId]->mReObj.find(mpAtlas->mvObjectMap[vObjByTIdLower[j]]->mnId);
                            if (sit != mpAtlas->mvObjectMap[ReId]->mReObj.end())
                            {
                                int sit_sec = sit->second;
                                mpAtlas->mvObjectMap[ReId]->mReObj.erase(mpAtlas->mvObjectMap[vObjByTIdLower[j]]->mnId);
                                mpAtlas->mvObjectMap[ReId]->mReObj.insert(make_pair(mpAtlas->mvObjectMap[vObjByTIdLower[j]]->mnId, sit_sec + 1));
                            }
                            else
                                mpAtlas->mvObjectMap[ReId]->mReObj.insert(make_pair(mpAtlas->mvObjectMap[vObjByTIdLower[j]]->mnId, 1));
                        }
                    }
                }
                else
                {
                    if (vObjByTId.size() >= 1)
                    {
                        ...
                        {
                            bool bFlag = mpAtlas->mvObjectMap[vObjByTId[i]]->DataAssociateUpdate(this, mCurrentFrame, image, 3); // 3 是指 T 方法.
                            ...
                            if (bFlag)
                            {
                                ...
                                if (vObjByTId.size() > i)
                                {
                                    ...
                                    {
                                        map<int, int>::iterator sit;
                                        sit = mpAtlas->mvObjectMap[nAssoByTId]->mReObj.find(mpAtlas->mvObjectMap[vObjByTId[j]]->mnId);
                                        if (sit != mpAtlas->mvObjectMap[nAssoByTId]->mReObj.end())
                                        {
                                            int sit_sec = sit->second;
                                            mpAtlas->mvObjectMap[nAssoByTId]->mReObj.erase(mpAtlas->mvObjectMap[vObjByTId[j]]->mnId);
                                            mpAtlas->mvObjectMap[nAssoByTId]->mReObj.insert(make_pair(mpAtlas->mvObjectMap[vObjByTId[j]]->mnId, sit_sec + 1));
                                        }
                                        else
                                            mpAtlas->mvObjectMap[nAssoByTId]->mReObj.insert(make_pair(mpAtlas->mvObjectMap[vObjByTId[j]]->mnId, 1));
                                    }
                                }
                                ...
                                if (vObjByTIdLower.size() >= 0)
                                {
                                    for (int j = 0; j < vObjByTIdLower.size(); j++)
                                    {
                                        ...
                                        map<int, int>::iterator sit;
                                        sit = mpAtlas->mvObjectMap[nAssoByTId]->mReObj.find(mpAtlas->mvObjectMap[vObjByTIdLower[j]]->mnId);
                                        if (sit != mpAtlas->mvObjectMap[nAssoByTId]->mReObj.end())
                                        {
                                            int sit_sec = sit->second;
                                            mpAtlas->mvObjectMap[nAssoByTId]->mReObj.erase(mpAtlas->mvObjectMap[vObjByTIdLower[j]]->mnId);
                                            mpAtlas->mvObjectMap[nAssoByTId]->mReObj.insert(make_pair(mpAtlas->mvObjectMap[vObjByTIdLower[j]]->mnId, sit_sec + 1));
                                        }
                                        else
                                            mpAtlas->mvObjectMap[nAssoByTId]->mReObj.insert(make_pair(mpAtlas->mvObjectMap[vObjByTIdLower[j]]->mnId, 1));
                                    }
                                }
            ...
            ObjectMapSingle->mnId = mpAtlas->mvObjectMap.size();
...
// BRIEF compute the mean and standard deviation of objects in the map.
    void Object_Map::ComputeMeanAndStandard()
    {
        ...
        // mean(3d center).
        mCenter3D = mSumPointsPos / (mvpMapObjectMappoints.size());//mynote
...
// BRIEF whether merge two potantial associated objes.
    void Object_Map::WhetherMergeTwoMapObjs(Atlas *mpAtlas)
    {
        map<int, int>::iterator sit;
//    for (sit = mReObj.begin(); sit != mReObj.end(); sit++)
        map<int,int> ReObj = this->mReObj;//mynote
        for (sit = ReObj.begin(); sit != ReObj.end(); sit++)
        {
            ...
            if (mpAtlas->mvObjectMap[nObjId]->bBadErase)
                continue;

            // note: double t-test.
            bool bDoubelTtest = this->DoubleSampleTtest(mpAtlas->mvObjectMap[nObjId]);
            bool bSametime = true;
            ...
            if((!bSametime || bDoubelTtest))
            {
                ...
                int nAppearTimes2 = mpAtlas->mvObjectMap[nObjId]->mObjectFrame.size();

                if (nAppearTimes1 > nAppearTimes2)
                {
                    this->MergeTwoMapObjs(mpAtlas->mvObjectMap[nObjId]);
                    this->ComputeMeanAndStandard();
                    this->IsolationForestDeleteOutliers();
                    mpAtlas->mvObjectMap[nObjId]->bBadErase = true;
                }
                else
                {
                    mpAtlas->mvObjectMap[nObjId]->MergeTwoMapObjs(this);
                    mpAtlas->mvObjectMap[nObjId]->ComputeMeanAndStandard();
                    mpAtlas->mvObjectMap[nObjId]->IsolationForestDeleteOutliers();
                    this->bBadErase = true;
                }
            }
        }
...
// BRIEF update object pose. Myplan 更新立方体位姿
...
//        cv::Mat pts1 = mSumPointsPos.clone(); // 创建pts的一个副本，以免修改原始数据
//        //个矩阵将用来存储 good_points 中的点的坐标。
