...
#ifndef ORB_SLAM3_LINEDETECTOR_H
#define ORB_SLAM3_LINEDETECTOR_H
...
// 3D line segment detector
    ...
    public:
        ...
        void RunLine3Dpp(std::vector<ORB_SLAM3::KeyFrame *> vpKFs);
        ...
        void LineFittingOffline(std::vector<ORB_SLAM3::KeyFrame *> vpKFs, ORB_SLAM3::Modeler *pModeler);

        // edge detection by EdgeDrawing
        void DetectEdgeMap(ORB_SLAM3::KeyFrame *kf);

        // 2D line segment detection by EDLines
        void DetectLineSegments(ORB_SLAM3::KeyFrame *kf);
        ...
        void
        LeastSquaresLineFit(Pixel *pixelChain, int initLength, float &u1, float &u2, float &u3, float &lineFitError);

        void LeastSquaresDepthFit(Pixel *pixelChain, int initLength, float la, float lb, float lc, float &u1, float &u2,
                                  float &depthFitError, ORB_SLAM3::KeyFrame *kf);

        float ComputePointDistance2Line(float a, float b, float c, Pixel pixel);

        float ComputePointDepth2Line(float a, float b, float c, float alpha, float beta, Pixel *pixelChain, Pixel pixel,
        ...
        void MergeLines(ORB_SLAM3::KeyFrame *kf, ORB_SLAM3::Modeler *pModeler);
...
#endif //ORB_SLAM3_LINEDETECTOR_H
